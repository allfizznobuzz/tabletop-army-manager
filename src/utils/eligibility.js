...
export const hasActiveOverrides = (ov) => {
  if (!ov) return false;
  return (ov.canLead && ov.canLead !== 'auto') || (ov.canBeLed && ov.canBeLed !== 'auto') || (ov.allowList && ov.allowList.length);
};
...
export const getOverrideSummary = (ov, resolveName) => {
  ...
  const parts = [];
  if (ov.canLead && ov.canLead !== 'auto') parts.push(`Lead ${ov.canLead === 'yes' ? '✓' : '✗'}`);
  if (ov.canBeLed && ov.canBeLed !== 'auto') parts.push(`Led ${ov.canBeLed === 'yes' ? '✓' : '✗'}`);
  const names = (ov.allowList || []).map(id => (resolveName ? resolveName(id) : id));
...
export const canAttach = (leader, unit, overridesMap, sourceCheck) => {
  ...
  const lOv = overridesMap?.[leader.id] || { canLead: 'auto', canBeLed: 'auto', allowList: [] };
  const uOv = overridesMap?.[unit.id] || { canLead: 'auto', canBeLed: 'auto', allowList: [] };

  // 1) Pairwise Allow
  if ((lOv.allowList || []).includes(unit.id) || (uOv.allowList || []).includes(leader.id)) return true;
