...
describe("attackMath.woundTarget", () => {
  ...
  describe("attackMath.computeDefenderSave", () => {
    it("applies AP correctly: 3+ with AP-2 => armourAfterAp 5+", () => {
      const r = computeDefenderSave("3+", -2, null);
      expect(r.armourAfterAp).toBe(5);
      expect(r.best).toBe(5);
      expect(r.used).toBe("armour");
    });

    it("prefers invuln on tie: 4+ armour after AP vs 4+ invuln", () => {
      const r = computeDefenderSave("4+", 0, "4+");
      expect(r.armourAfterAp).toBe(4);
      expect(r.invuln).toBe(4);
      expect(r.best).toBe(4);
      expect(r.used).toBe("invuln");
    });

    it("no invuln uses armour after AP", () => {
      const r = computeDefenderSave("2+", -1, null);
      expect(r.armourAfterAp).toBe(3);
