...
// NOTE: GameSession is required after mocks below
...
describe("Weapon carrier defaults", () => {
  test("Power Fist defaults to 1; Chainsword defaults to 2 via 2x prefix", async () => {
    ...
    const fistRow = await screen.findByRole("button", { name: /power fist/i });
    fireEvent.click(fistRow);
    const input1 = await screen.findByLabelText(/models in range/i);
    expect(input1).toHaveValue(1);

    // Close and open Chainsword
    fireEvent.click(fistRow); // close
    const chainRow = await screen.findByRole("button", { name: /chainsword/i });
    fireEvent.click(chainRow);
    const input2 = await screen.findByLabelText(/models in range/i);
